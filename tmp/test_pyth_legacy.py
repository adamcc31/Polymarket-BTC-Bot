...
def fetch_pyth_price_legacy(timestamp_sec):
    # Pyth XC-Mainnet legacy endpoint (very clean JSON)
    url = "https://xc-mainnet.pyth.network/api/get_price_feed"
    params = {
    ...
    try:
        ...
        # 404/Empty is possible if timestamp has no exact match
        ...
        print(f"Pyth Legacy API Error for {timestamp_sec}: {e}")
        return None
...
def test_pyth_alignment():
    ...
    for mid, start_ms, res_ms, target in test_cases:
        ...
        # Get start price (Strike) - for Above $82k, strike is static
        if mid == "1088018":
            strike = 82000.0
        else:
            strike = fetch_pyth_price_legacy(start_ms // 1000)
            
        # Get res price (Final)
        final = fetch_pyth_price_legacy(res_ms // 1000)
